...
RSpec.describe RadioStation, type: :model do
  ...
  describe "validations" do
    ...
    context "youtube station" do
      subject { build(:radio_station, source_type: "youtube") }
      ...
      it "does not require stream_url" do
        station = build(:radio_station, source_type: "youtube", stream_url: nil)
        station.valid?
  ...
  describe "#youtube?" do
    it "returns true for youtube source_type" do
      expect(build(:radio_station, source_type: "youtube")).to be_youtube
    end

    it "returns false for stream source_type" do
      expect(build(:radio_station, :stream)).not_to be_youtube
    end
  end

  describe "#stream?" do
    it "returns true for stream source_type" do
      expect(build(:radio_station, :stream)).to be_stream
    end

    it "returns false for youtube source_type" do
      expect(build(:radio_station, source_type: "youtube")).not_to be_stream
    end
  end
end
